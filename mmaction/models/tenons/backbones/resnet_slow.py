...
from .decouplenorm import TemporalNorm, SpatialNorm
...
from ..spatial_temporal_modules.temporal_pyramid import TemporalPyramidModule
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.planes = planes
        self.tsc = tsc and if_inflate
        temporal_norm = tnorm and if_inflate
        spatial_norm = snorm and if_inflate
        ...
        self.bn1 = TemporalNorm(planes) if temporal_norm else nn.BatchNorm3d(planes)
        self.bn2 = SpatialNorm(planes) if spatial_norm else nn.BatchNorm3d(planes)
...
class ResNet_SlowFast(nn.Module):
    ...
    def __init__(self,
        ...
        if self.tpm:
            self.TPM = TemporalPyramidModule(256)
                ...
                if self.affine_bn:
                    for m in self.modules():
                        if isinstance(m, nn.BatchNorm3d):
                            ...
                            # According to NonLocal Network
                            std = torch.sqrt(running_var + 1e-5)
                            new_weight = weight / std
                            new_bias = bias - running_mean * weight / std
